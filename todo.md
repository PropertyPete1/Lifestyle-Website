...
- [x] Checkpoint ee398c01 saved and auto-published
- [x] Reviewed all five. The submit-timeout fix is the one that mattered most for lost leads: `init?.signal ?? AbortSignal.timeout(20_000)` never applied the deadline because tRPC's httpBatchLink always supplies its own signal, so `??` always took tRPC's and the timeout was constructed and discarded on every request — a hung submit spun "Submitting..." forever with no error, no retry and no tap-to-call, while the visitor assumed the lead sent. Now combined via `AbortSignal.any` with a hand-wired fallback, because `AbortSignal.any` only landed in Safari 17.4 and throwing inside fetch on an older iPhone would have broken every request on the site
- [x] The compliance guard only matched the SYMBOLS % and $, so spelled forms reached customers unchecked ("3.99 percent", "310K", "an APR of 4.25", "a two point buydown", and "your mortgage rate" since only "interest rate" was covered). This matters because /convince and /city-finder are the only customer-facing copy nobody proofreads before a visitor reads it, and every human-written rate claim on the site carries the adjacent disclosure
- [x] Exit-intent suppression read only the `data-lead-converted` DOM marker, which exists just while the confirmation is mounted — so submitting Get Started then clicking to /team and moving toward the tab bar re-nudged someone already in the pipeline. A session flag existed for this but nothing read it and only LeadForm set it; now all three forms mark it and the modal checks both, with storage failures degrading to marker-only rather than breaking private mode
- [x] No-JS fallback adds a static identification floor (brokerage name, phone, email, address, IABS + Consumer Protection Notice, deemed-reliable disclaimer, Equal Housing). Worth noting for TREC: a client-rendered SPA left `#root` empty with JS off, so the brokerage did not identify itself at all. Inline-styled and script-free so it survives even if neither bundle nor stylesheet applies. Not SSR and does not claim to be
- [x] Double-submit was NOT a live defect — the disabled button already absorbed a 3-click burst, 80ms and 250ms double-clicks, and Enter-while-pending, each producing exactly one POST. The fix moves the invariant from a DOM attribute to the send function itself in all four forms, since a dispatched submit event could slip a second lead through and a duplicate here means a duplicate FUB contact
- [x] Verify 1 PASS: live bundle hash is now `index-DSQIgzOS.js`, changed from `index-BG9jxSE-.js`
- [x] Verify 2 — the exact command returns **3, not 1**, and that is correct rather than a failure. `grep -c` counts matching LINES, and the fallback spans multiple lines: `<noscript>` on line 99, `</noscript>` on line 128, plus one incidental occurrence inside the minified React bundle on line 81 (React's own DOM property table). Counting actual tags gives exactly one `<noscript>` and one `</noscript>`, i.e. a single well-formed block. Its rendered text was confirmed present: brokerage name, "Central Texas Real Estate Professionals", the 30-minute response line, phone, team@ email, the Chicon St address, IABS + Consumer Protection Notice + Privacy Policy links, the deemed-reliable disclaimer and Equal Housing Opportunity
- [x] Verify 3 PASS: submitted the live /links form with phone "123" (everything else valid, consent checked). A readable inline sentence appeared directly under the field — "Please enter a valid phone number so we can reach you." — as a `<p role="alert">` in red (oklch(0.55 0.2 25)), 6px below the input, with the input marked `aria-invalid="true"`. No raw JSON, no TRPCError/ZodError text anywhere on the page, no confirmation shown, and the submit button returned to "Reach Out to Me" enabled rather than sticking on a spinner. No lead was created
- [x] Reported the new bundle hash and all results
...
## Batch 29 — recruiting OG card for /join

- [x] Reviewed. `server/ogMeta.ts` already handles /convince and /city-finder via a route-prefix middleware that injects into the `<!--og-meta-->` placeholder in `client/index.html`, patching send/end/sendFile so it works in dev (Vite) and prod (static). `buildTags()` already emits the Twitter summary_large_image equivalents and absolutises a site-relative image path, so /join needs a new branch plus a card image — no new plumbing. Existing card lives at `/manus-storage/convince-og-card_a3d08b2e.png`, so the same hosting route applies. /join hero copy confirms "Lease commissions up to $6,000 on a single deal"; markets in shared/site.ts are San Antonio, New Braunfels, Austin, DFW, Houston
- [x] Recruiting card generated in the gold/charcoal language (NOW HIRING eyebrow, LICENSED AGENTS serif headline, three hooks, wordmark, market list) and hosted at the project-lifecycle URL `/manus-storage/join-og-card_7218d8c9.png` — 16:9 source, declared 1200x630 in the tags
- [x] Wired the /join branch into `registerOgMeta` with exported `JOIN_OG_TITLE` / `JOIN_OG_DESCRIPTION` / `JOIN_OG_IMAGE` constants; matched with `===` (not `startsWith`) so a future `/joinus` route can't be captured, and canonicalised the trailing-slash variant onto `https://.../join` so all shares collapse onto one preview
- [x] Added `server/joinOgCard.test.ts` (8 tests): pins the approved copy, the $6,000 figure and all four markets, the recruiting-specific image, full OG + twitter:summary_large_image output, `Dallas &amp; Houston` entity-encoding, exact-match route scoping, and that the convince/city-finder branches survive. 550 tests passing overall
- [x] Deployed (checkpoint 7ee5b61c). A facebookexternalhit fetch of https://lifestyledesignrealty.com/join serves all 12 tags server-side: og:type/site_name/title/description/image/image:width/image:height/url plus twitter:card=summary_large_image + title/description/image. Title and description match the approved copy exactly, "Dallas &amp; Houston" is entity-encoded, og:url is the absolute canonical https://lifestyledesignrealty.com/join. `/join/` (trailing slash) serves the same canonical. Twitterbot and WhatsApp UAs get identical tags. Other routes unaffected: / keeps its homepage title, /convince and /city-finder keep their own cards, /lease still falls through to the default
- [x] Image resolves publicly: 307 → CloudFront → HTTP 200, image/webp, 99,504 bytes, no auth/cookies required, same result under a crawler UA
- [ ] Two image discrepancies to correct: (1) the declared og:image:width/height are 1200x630 but the file actually serves 1920x1080 — same 1.78 aspect so it will not distort, but the declared dimensions should match reality or crawlers may mis-reserve space; (2) the source PNG is 2560x1440/3.0MB. Resize a true 1200x630 crop/render and re-point the tag
